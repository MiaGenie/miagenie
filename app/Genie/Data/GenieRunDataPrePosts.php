...
class GenieRunDataPrePosts implements GenieRunDataContract
{
    use GenieParser;
    ...
    public function nextStep(?RuleStep $lastStep = null): ?RuleStep
    {
        ...
        if ($lastRunResponse && ($lastRunResponse?->status->isError() || !$lastRunResponse?->status->isComplete())
            ...
            $this->nextStep = match ($lastStep?->rule_sub_type->name) {
                null, 'PRE_POSTS_INITIAL' => $this->getNextStep(),
                'PRE_POSTS' => $this->getNextStepIterator()
            };
        }
    ...
    {
        $stepRunIterators = RunDraft::where('run_id', $this->run->id)->get();
    ...
    {
        $doneIterators = RunDraft::whereHas('runDraftResponses.runResponse.step', function ($query) use ($step) {
