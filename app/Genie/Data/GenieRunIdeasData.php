...
use App\Models\RunFieldIterator;
use App\Models\Run;
...
class GenieRunIdeasData implements GenieRunDataContract
{
    use GenieParser;
    ...
    public function nextStep(?RuleStep $lastStep = null): ?RuleStep
    {
        ...
        if ($lastRunResponse && ($lastRunResponse?->status->isError() || !$lastRunResponse?->status->isComplete())
            ...
            $this->nextStep = match ($lastStep?->rule_sub_type->name) {
                null, 'IDEAS_SIMPLE' => $this->getNextStep(),
    ...
    {
        $fieldValues = collect($this->run->runIdea->strategy->content[$step->dependsOnField->code_name] ?? []);
