...
import {
    convertLaravelErrorsToString,
    loadScriptsFromString,
} from "@/MixpostEnterprise/helpers";
import WorkspaceLayout from "@/Layouts/MixpostEnterprise/Workspace.vue";
...
const props = defineProps({
    ...
    paymentSupportTrialing: {
        type: Boolean,
        required: true,
    },
    paymentSupportCoupon: {
        type: Boolean,
        required: true,
    },
});
...
const getTrialDays = () => {
    if (!props.billingConfigs.generic_trial && !props.paymentSupportTrialing) {
        return 0;
    }
    ...
    if (
        props.workspace.generic_subscription &&
        props.workspace.generic_subscription.has_trial
    ) {
        return 0;
    }

    return parseInt(props.billingConfigs.trial_days);
};
...
const buttonLabel = computed(() => {
    ...
    if (trialDays) {
        return $t("enterprise-subscription.start_trial", { days: trialDays });
    }

    if (props.subscription && !isCanceled(props.subscription.status)) {
        return $t("enterprise-subscription.change_plan");
    }

    return $t("enterprise-general.subscribe");
});
...
onMounted(() => {
    document.addEventListener(
        "closePaymentDetailsModal",
        handleClosePaymentDetailsModalEvent,
    );
});
...
const newSubscription = () => {
    ...
    axios
        .post(
            route(`${routePrefix}.workspace.subscription.new`, {
        ...
        .catch((error) => {
            ...
            if (error.response.status === 422) {
                form.setError(
                    "error",
                    convertLaravelErrorsToString(error.response.data.errors),
                );
            }
...
const swapSubscriptionPlan = () => {
    form.post(
        route(`${routePrefix}.workspace.subscription.changePlan`, {
            workspace: activeWorkspaceId.value,
        }),
        {
            preserveScroll: true,
...
const title = () => {
    if (props.subscription && !isCanceled(props.subscription.status)) {
        return $t("enterprise-subscription.change_sub_plan");
    }
...
<template>
    ...
    <div class="w-full mx-auto">
        <Panel>
            ...
            <Plans
                :current-plan="subscription ? subscription.plan : null"
                :billing-configs="billingConfigs"
                :plans="plans"
                :plansInfo="plansInfo"
                @change="updateForm"
            />
        ...
        <template v-if="paymentSupportCoupon">
            <Panel class="mt-lg">
                <template #title>
                    {{ $t("enterprise-subscription.use_coupon") }}
                </template>
                ...
                <HorizontalGroup>
                    <Input
                        ...
                        type="text"
                        :placeholder="
                            $t('enterprise-subscription.insert_coupon')
                        "
                    />
                </HorizontalGroup>
            </Panel>
        ...
        <template v-if="subscription">
            ...
            >
                ...
                <span
                    v-if="!isCanceled(subscription.status)"
                    class="link-primary"
                    >{{ $t("enterprise-plan.keep_old_plan") }}</span
                >
                <span
                    v-if="isCanceled(subscription.status)"
                    class="link-primary"
                    >{{ $t("enterprise-dashboard.back_billing") }}</span
                >
            </PureButtonLink>
        </template>
        ...
        <PrimaryButton
            ...
            :is-loading="newSubscriptionIsLoading || form.processing"
            size="sm"
            class="mt-lg"
            @click="submit"
        >
    ...
    >
        <template #header>
            {{ $t("enterprise-dashboard.payment_details") }}
        </template>
        ...
        <template #footer>
            <SecondaryButton @click="closePaymentDetailsModal">{{
                $t("general.cancel")
            }}</SecondaryButton>
        </template>
    </DialogModal>
