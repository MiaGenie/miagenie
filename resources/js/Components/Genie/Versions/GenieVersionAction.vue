...
const confirmDeleteGenieVersion = () => {
    confirmation()
        .title($t('genie.delete_version'))
        .description($t('genie.delete_version_confirm'))
        .destructive()
        .onConfirm((dialog) => {
            deleteGenieVersionAfterConfirmed(dialog);
        })
        .show();
}

const deleteGenieVersionAfterConfirmed = (dialog) => {
...
<template>
    <Flex :responsive="false" class="items-center">
        <template v-if="destroy">
            <DangerButton @click="confirmDeleteGenieVersion" size="sm">
                <template #icon>
                    <Trash/>
                </template>
            </DangerButton>
