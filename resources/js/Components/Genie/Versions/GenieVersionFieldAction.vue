...
const confirmDeleteGenieGroup = () => {
    confirmation()
        .title($t('genie.delete_group'))
        .description($t('genie.delete_group_confirm'))
        .destructive()
        .onConfirm((dialog) => {
            deleteGenieGroupAfterConfirmed(dialog);
        })
        .show();
}

const deleteGenieGroupAfterConfirmed = (dialog) => {
...
<template>
    <Flex :responsive="false" class="items-center">
        <template v-if="destroy">
            <DangerButton @click="confirmDeleteGenieGroup" size="sm">
                <template #icon>
                    <Trash/>
                </template>
            </DangerButton>
