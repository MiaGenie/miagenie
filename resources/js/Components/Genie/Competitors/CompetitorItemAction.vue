...
const getRoute = (name) => {
    switch (name) {
        case 'edit':
            return route('genie.competitors.edit', {
                workspace: workspaceCtx.id,
                competitor: props.itemId
            });
        case 'delete':
            return route('genie.competitors.delete', {
                workspace: workspaceCtx.id,
                competitor: props.itemId
            });
        default:
...
const deleteCompetitorAfterConfirmed = (dialog) => {
    ...
    router.delete(getRoute('delete'), {
        onSuccess() {
            ...
            emit('onDelete')
            emitter.emit('competitorDeleted', props.itemId);
            dialog.reset();
        },
        onError(errors) {
            onError(errors, () => {
        ...
        <div class="flex flex-row items-center justify-end gap-xs sm:hidden">
            ...
            <Dropdown placement="bottom-end">
                ...
                <template #content>
                    ...
                    <DropdownItem @click="confirmDeleteCompetitor" as="button">
                        <template #icon>
