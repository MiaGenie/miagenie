...
<template>
    <WarningButton @click="openModal">{{ $t('enterprise-subscription.add_sub') }}</WarningButton>

    <DialogModal :show="modal" max-width="md" @close="closeModal">
        <template #header>{{ $t('enterprise-subscription.add_sub') }}</template>

        <template #body>
            <template v-if="plans.length">
                <Alert variant="warning" :closeable="false">
                    {{ $t('enterprise-subscription.cancel_delete_sub') }}
                </Alert>
                ...
                <div class="mt-lg">
                    <label>
                        ...
                        {{ $t('enterprise-subscription.keep_sub_trial') }}
                    </label>
                </div>

                <template v-if="!form.keep_prev_trial_ends_at">
                    <VerticalGroup class="mt-lg">
                        <template #title> {{ $t('enterprise-subscription.trial_days') }}</template>
                        <div class="w-full">
                            <div class="text-gray-500 mb-1">{{ $t('enterprise-subscription.empty_config') }}</div>
