<script setup>
import { computed, inject, onMounted, reactive, watch } from "vue";
import { useI18n } from "vue-i18n";
import VerticalGroup from "@/Components/Layout/VerticalGroup.vue";
import Flex from "@/Components/Layout/Flex.vue";
import Radio from "@/Components/Form/Radio.vue";
import LabelGroup from "@/MixpostEnterprise/Components/Surface/LabelGroup.vue";
import { find } from "lodash";

const { t: $t } = useI18n();

const props = defineProps({
    currentPlan: {
        type: [Object, null],
        default: null,
    },
    plans: {
        type: Array,
        required: true,
    },
    plansInfo: {
        type: Array,
        required: true,
    },
    billingConfigs: {
        type: Object,
        required: true,
    },
});

const emit = defineEmits(["change"]);

const description = (plan_id) => {
    return find(props.plansInfo, ["plan_id", plan_id]).description;
};

const billingCycles = computed(() => {
    if (props.billingConfigs.billing_cycle === "monthly") {
        return [
            { value: "monthly", name: $t("enterprise-subscription.monthly") },
        ];
    }

    if (props.billingConfigs.billing_cycle === "yearly") {
        return [
            { value: "yearly", name: $t("enterprise-subscription.yearly") },
        ];
    }

    return [
        { value: "monthly", name: $t("enterprise-subscription.monthly") },
        { value: "yearly", name: $t("enterprise-subscription.yearly") },
    ];
});

const form = reactive(
    props.currentPlan
        ? {
              cycle: props.currentPlan.cycle,
              plan_id: props.currentPlan.id,
          }
        : {
              cycle: billingCycles.value[0].value,
              plan_id: props.plans.length > 0 ? props.plans[0].id : null,
          },
);

const isYearly = computed(() => form.cycle === "yearly");

onMounted(() => {
    emit("change", form);
});

watch(form, (value) => {
    emit("change", value);
});
</script>
<template>
    <div>
        <VerticalGroup>
            <template #title>{{
                $t("enterprise-subscription.billing_cycle")
            }}</template>

            <Flex :col="true" class="w-full">
                <template v-for="cycle in billingCycles" :key="cycle.value">
                    <LabelGroup :active="cycle.value === form.cycle">
                        <Radio
                            v-model:checked="form.cycle"
                            :value="cycle.value"
                        />
                        {{
                            $t("enterprise-subscription.pay_cycle", {
                                cycle: cycle.name,
                            })
                        }}
                    </LabelGroup>
                </template>
            </Flex>
        </VerticalGroup>

        <VerticalGroup :forceFullWidth="true" class="mt-lg">
            <template #title>
                <div>{{ $t("plan.plan") }}</div>
            </template>

            <Flex
                :wrap="true"
                :responsive="false"
                class="columns-3 w-full justify-between items-center"
            >
                <template v-for="plan in plans" :key="plan.id">
                    <Flex :col="true" :responsive="false" class="max-w-[30%]">
                        <LabelGroup :active="plan.id === form.plan_id">
                            <span class="flex justify-between items-center">
                                <span>
                                    <Radio
                                        v-model:checked="form.plan_id"
                                        :value="plan.id"
                                    />
                                    {{ plan.name }}
                                    <div
                                        v-html="description(plan.id)"
                                        class="plan_description"
                                    ></div>
                                    <span v-if="isYearly && !plan.is_free"
                                        >(yearly)</span
                                    >
                                </span>

                                <template v-if="!plan.is_free">
                                    <span v-if="isYearly">
                                        {{ plan.price.yearly.amount }}
                                        {{ billingConfigs.currency }}
                                    </span>

                                    <span v-else
                                        >{{ plan.price.monthly.amount }}
                                        {{ billingConfigs.currency }}</span
                                    >
                                </template>
                            </span>
                        </LabelGroup>
                    </Flex>
                </template>
            </Flex>
        </VerticalGroup>
    </div>
</template>
